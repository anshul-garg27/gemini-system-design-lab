...
def db_operation(commit=True, max_retries=10):
    """
    Decorator for database operations with retry logic for lock contention.
    
    Args:
        commit: Whether to commit the transaction (default: True)
        max_retries: Maximum number of retries for locked database (default: 10)
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    if 'locked' in str(e).lower():
                        ...
                        jitter = random.uniform(0, 0.05)  # Add 0-50ms random jitter
                        wait_time = base_wait + jitter
                        logger.warning(f"Database locked in {func.__name__}, retry {attempt + 1}/{max_retries} after {wait_time:.2f}s")
            ...
            logger.error(f"Database locked in {func.__name__} after {max_retries} retries")
...
class UnifiedDatabase:
    ...
    def _get_connection(self):
        ...
        if not hasattr(self._local, 'conn'):
            ...
            # Optimize WAL and write performance
            self._local.conn.execute("PRAGMA wal_autocheckpoint=1000")  # Checkpoint every 1000 pages
            self._local.conn.execute("PRAGMA synchronous=NORMAL")      # Faster writes (safe with WAL)
            self._local.conn.execute("PRAGMA cache_size=-64000")       # 64MB cache
            self._local.conn.execute("PRAGMA temp_store=MEMORY")       # Temp tables in memory
            # Enable foreign keys
            self._local.conn.execute("PRAGMA foreign_keys = ON")
            # Optimize for concurrent access
            self._local.conn.execute("PRAGMA busy_timeout = 30000")  # 30 seconds
    ...
    def get_connection(self):
        ...
        Creates a NEW connection each time for backward compatibility with code that calls conn.close().
        """
        ...
        # Enable WAL mode and optimizations for this connection too
    ...
    @db_operation()
    async def create_job(self, cursor, job_id: str, topic_id: str, topic_name: str, status: str):
    ...
    @db_operation()
    async def update_job_status(self, cursor, job_id: str, status: str):
        """Update job status."""
        cursor.execute("""
    ...
    @db_operation()
    async def create_task(self, cursor, task_id: str, job_id: str, platform: str, format: str, status: str):
        """Create a new task."""
        cursor.execute("""
    ...
    @db_operation()
    async def update_task_status(self, cursor, task_id: str, status: str, result: str = None, error: str = None):
        """Update task status."""
        cursor.execute("""
    ...
    @db_operation()
    async def save_result(self, cursor, result_id: str, job_id: str, platform: str, format: str, content: str):
    ...
    @db_operation()
    async def save_prompt(self, cursor, prompt_id: str, job_id: str, platform: str, format: str, prompt_text: str):
    ...
    def get_topics_paginated(self, cursor, limit: int = 20, offset: int = 0, 
                           ...
                           tag: str = None, technology: str = None,
        ...
        if technology:
            # Filter by technology in JSON array
        ...
        valid_sort_fields = ["title", "created_date", "updated_date", "complexity_level", "company", "difficulty"]
        ...
        query = f"""
            SELECT topics.*, 
                   topic_status.status as processing_status, 
                   topic_status.error_message
            FROM topics
            {join_condition}
            {where_clause}
            {order_clause}
    ...
    def get_topics_count(self, cursor, search: str = None, category: str = None, 
                        subcategory: str = None, status: str = None, 
                        complexity: str = None, company: str = None,
                        tag: str = None, technology: str = None) -> int:
    ...
    @db_operation(commit=False)
    def get_filter_options(self, cursor) -> Dict[str, Any]:
        """Get all available filter options for topics."""
        # Get unique categories
        cursor.execute("SELECT DISTINCT category FROM topics WHERE category IS NOT NULL ORDER BY category")
        categories = [row[0] for row in cursor.fetchall()]
        
        # Get unique subcategories
        cursor.execute("SELECT DISTINCT subcategory FROM topics WHERE subcategory IS NOT NULL ORDER BY subcategory")
        subcategories = [row[0] for row in cursor.fetchall()]
        
        # Get unique companies
        cursor.execute("SELECT DISTINCT company FROM topics WHERE company IS NOT NULL ORDER BY company")
        companies = [row[0] for row in cursor.fetchall()]
        
        # Get unique complexity levels
        cursor.execute("SELECT DISTINCT complexity_level FROM topics WHERE complexity_level IS NOT NULL ORDER BY complexity_level")
        complexity_levels = [row[0] for row in cursor.fetchall()]
        
        # Get all unique technologies (from JSON arrays)
        cursor.execute("SELECT DISTINCT technologies FROM topics WHERE technologies IS NOT NULL")
        technologies_set = set()
        for row in cursor.fetchall():
            try:
                techs = json.loads(row[0]) if isinstance(row[0], str) else row[0]
        ...
        # Get all unique tags (from JSON arrays)
        cursor.execute("SELECT DISTINCT tags FROM topics WHERE tags IS NOT NULL")
        tags_set = set()
        for row in cursor.fetchall():
            try:
                tag_list = json.loads(row[0]) if isinstance(row[0], str) else row[0]
        ...
        cursor.execute("SELECT DISTINCT status FROM topic_status WHERE status IS NOT NULL ORDER BY status")
    ...
    def save_topic_status_batch(self, cursor, topics: List[tuple]) -> int:
        ...
        Much faster than calling save_topic_status() in a loop.
        
        Args:
            topics: List of (title, status, error_message) tuples
        ...
        if 'original_title' in columns:
            # New schema - batch insert/update
            for title, status, error_message in topics:
            ...
            # Old schema - batch insert or replace
            cursor.executemany("""
        ...
        logger.info(f"Batch saved {saved_count} topic statuses")
        return saved_count
    ...
    def save_topic_status(self, cursor, title: str, status: str, error_message: str = None) -> bool:
        ...
        For batch operations, use save_topic_status_batch() instead.
